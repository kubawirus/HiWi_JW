...
mechanism = 2
remove = 1
inactiv = 3 # inactiv = 1 no deactivation, inactiv > 1 how many cycles should reactor work with decreasing surface every cycle.
...
result_dict_gas = {}
list_for_results_gas = [ "moles", "mflow", "mol_flow", "u", "state_list", "mass", "pressure", "temp", "prod_rates_gas"]
...
result_dict_surf = {}
list_for_results_surf = ["mflow_surf", "u_surf", "state_list_surf", "surf_area_n", "depo", "depo_r", "mass_depo", "sum_depo", "surf_inactiv", "cat_area_per_vol", "prod_rates_surf", "cov"]
...
for k in range (inactiv):
    #---- Gas Reactor ----#
    result_dict_gas["state_list"][k].append(reactor.thermo.TPY)
    result_dict_gas["pressure"][k].append(reactor.thermo.P)
    result_dict_gas["temp"][k].append(reactor.T)
    result_dict_gas["mflow"][k].append(mass_flow_rate_0)
    result_dict_gas["u"][k].append(u_0)
    result_dict_gas["mass"][k].append(reactor.mass)

    #---- Surface Reactor ----#
    result_dict_surf["mflow_surf"][k].append(mass_flow_rate_0 * beta)
    result_dict_surf["u_surf"][k].append(u_0_surf)
    result_dict_surf["surf_area_n"][k].append(cat_surf)
...
for k in range (inactiv):
    ...
    for n in range(n_steps):
        ...
        result_dict_gas["pressure"][k].append(P)
        result_dict_gas["temp"][k].append(T)

        # update coverages
        for i, j in enumerate(surf.species_names):
             result_dict_surf["cov"][k][j].append(surf.coverages[i])
        ...
        if remove == 1:
            ...
            result_dict_gas["mass"][k].append(M)
            result_dict_gas["mflow"][k].append(M_FLOW)
            # mol_flow[n + 1] = mass_flow_now / reactor.thermo.mean_molecular_weight
            result_dict_gas["state_list"][k].append((T,P,Y))
            result_dict_surf["mass_depo"][k].append(0.0)
            ...
            for j, compound in enumerate(reactor.thermo.species()):
                ...
                if reactor.thermo.molecular_weights[j] > 80.00:
                    ...
                    if name_Y_excl not in result_dict_surf["depo"][k].keys():
                        result_dict_surf["depo"][k][name_Y_excl] = 0.0
                        result_dict_surf["depo_r"][k][name_Y_excl] = []
                    ...
                    result_dict_surf["mass_depo"][k][-1] += mass_flow_rate_down
                    # And whole summed deposited mass
                    result_dict_surf["sum_depo"][k] += mass_flow_rate_down
                    ...
                    result_dict_surf["depo"][k][name_Y_excl] += mass_flow_rate_down
                    result_dict_surf["depo_r"][k][name_Y_excl].append(mass_flow_rate_down)

            # Set a new value for mass flow in the upstream
            m.mass_flow_rate = M_FLOW
            m_surf.mass_flow_rate = M_FLOW * beta
            result_dict_gas["mass"][k][-1] = M
            result_dict_gas["mflow"][k][-1] = M_FLOW
            result_dict_surf["mflow_surf"][k][-1] = M_FLOW * beta
            result_dict_gas["u"][k].append(M_FLOW/(reactor.density * area_react))
            result_dict_surf["u_surf"][k].append( (M_FLOW * beta )/ (reactor.density * area_cat))
            result_dict_gas["state_list"][k][-1] = ((T, P, Y))
            upstream.syncState()
            ...
            ## Problem: How to bind the deacrising activity of surface with increasing deposition of heavy compounds.
                result_dict_surf["cat_area_per_vol"][k+1].append( result_dict_surf["cat_area_per_vol"][k][n] *  (1 - (result_dict_surf["mass_depo"][k][n] * alpha)))
                result_dict_surf["surf_inactiv"][k+1].append(result_dict_surf["cat_area_per_vol"][k+1][n] * cat_vol)
            #----------------------------------------------------------------------------#
        else:
            #Baustelle!
            result_dict_gas["mass"][k].append(M)
            result_dict_gas["mflow"][k].append(M_FLOW)
            result_dict_surf["mflow_surf"][k].append(M_FLOW * beta)
            result_dict_gas["u"][k].append(M_FLOW/(reactor.density * area_react))
            result_dict_surf["u_surf"][k].append( (M_FLOW * beta )/ (reactor.density * area_cat))
            result_dict_gas["state_list"][k].append(STATE_LIST)
            upstream.syncState()
        ...
        for i, j in enumerate(rsurf.kinetics.kinetics_species_names):
            if j not in result_dict_surf["prod_rates_surf"][k].keys():
                result_dict_surf["prod_rates_surf"][k][j] = []
            result_dict_surf["prod_rates_surf"][k][j].append(rsurf.kinetics.net_production_rates.T[i])

        for i, j in enumerate(reactor.kinetics.kinetics_species_names):
            if j not in result_dict_gas["prod_rates_gas"][k].keys():
                result_dict_gas["prod_rates_gas"][k][j] = []
            result_dict_gas["prod_rates_gas"][k][j].append(reactor.kinetics.net_production_rates.T[i])
#----------------------------------------------------------------------------#
    ...
    #     f.close()
        ...
        depo_comp_name = max(result_dict_surf["depo"][k],key=result_dict_surf["depo"][k].get)
        print("\nThe largest fraction of deposition in" , k , " cycle is ", depo_comp_name)
    ...
    ax1.plot(z, result_dict_gas["pressure"][k])  # [s[1] for s in state_list]
    ax1.set_ylabel('Pressure [Pa]')

    ax2.plot(z, result_dict_gas["temp"][k])
    ax2.set_ylabel('Temperature [K]')

    ax3.plot(z, result_dict_gas["u"][k])
    ...
    ax4.plot(z, result_dict_gas["mflow"][k])
    ax4.set_ylabel('mass flow rate [kg/s]')

    ax6.plot(z, result_dict_gas["mass"][k])
    ...
    if remove == 1:
        ...
        ax7.plot(z[1:], result_dict_surf["mass_depo"][k])
        ax7.set_ylabel('sum of depo [kg/s]')

        ax8.plot(z[1:], result_dict_surf["depo_r"][k][depo_comp_name])
    ...
    # Fulfill the list
    for val in (result_dict_gas["state_list"][k]):
    ...
    cov1 = result_dict_surf["cov"][k].get('PT(S)')
    cov2 = result_dict_surf["cov"][k].get('H(S)')
    ax14.plot(z[1:], cov1)
    ...
    ax16.plot(z[1:], result_dict_surf["prod_rates_surf"][k]['C3H8'])
    ax17.plot(z[1:], result_dict_surf["prod_rates_surf"][k]['C3H6'])
    ...
    ax18.plot(z[1:], result_dict_gas["prod_rates_gas"][k]['C3H8'])
    ax19.plot(z[1:], result_dict_gas["prod_rates_gas"][k]['C3H6'])
    ...
    mass_depo_ext = ( result_dict_surf["sum_depo"][k] / t_react_sum ) * 3600 * 1000
    print('\nDeposition in reactor ', mass_depo_ext , 'g/h')
    # Check if sum depo is equal to mass flow rate difference:
    print("\n check sum : ", (result_dict_surf["sum_depo"][k] - ( mass_flow_rate_0 - result_dict_gas["mflow"][k][-1])))
